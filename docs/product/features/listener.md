...
On `/speak`, a live two-color waveform shows who is speaking with the rolling transcript above it, and controls are **Pause** / **Mute** / **End**; **Close** leaves at any time. In the corner orb, the orb itself is the who-is-speaking signal (its glow color and swell). Above it floats a **minimalist translucent panel** (frosted, `bg-paper/75` + blur) that streams the Coach's current line as it speaks (with the last finished line held so it never blanks mid-thought) and shows your own words faintly while you talk — so you always know what it's doing without a full transcript view. Next to the status label, a **"what it knows"** toggle expands that panel to show the context the session opened with, as **labeled sections** — "Who it's being," "What you last talked about," "Left open from last time," etc. (`mintRealtimeSession` returns a `contextSources: {label, detail}[]` alongside the model's own `instructions`; the panel renders the former, never the latter — the raw prompt mixes real context with the model's self-instructions on how to open the call, which reads like a debug dump, not something a person asked to see). Under the orb sit three small icon buttons: **mute** (mic icon toggle), **toss** (✕ — discard the conversation: the session ends as `tossed`, the memory backbone still summarizes it per [ADR 0023](../../decisions/0023-listener-memory-backbone.md), but the Center never runs and no report shows, straight back to the pill), and **end-and-file** (✓). Ending runs the [Center](the-center.md) ("Filing what you shared…") and then shows the **filing report** — full-screen on `/speak`, a compact corner panel for the orb. The chat toggle sits right beside the idle pill in the corner row; it steps back while the orb owns the corner.
...
- **Talking about the app itself, not your life.** Fixed 2026-07-23: if a call included the person narrating a bug or feature request for LifeGuide out loud, the summarizer had no way to tell that apart from a personal topic, and it would resurface next call as "worth a light check-in." The `listenerSummary` system prompt now explicitly excludes app/product/meta talk from `summary`/`topics`/`open_threads`.

## 7. AI involvement

Three model touchpoints: (1) the **realtime conversation** — OpenAI Realtime (`gpt-realtime-mini`, the `voice` task) bound to `LISTENER_INSTRUCTIONS` (optionally grounded in the last call's summary — ARI-23), with semantic-VAD turn detection and streaming input transcription (see [`../decisions/0004-voice-stack-and-levels.md`](../decisions/0004-voice-stack-and-levels.md)); (2) the post-call **Center** fan-out (see [`the-center.md`](the-center.md)); (3) the post-call **memory-backbone summary** — `openai/gpt-4o-mini`, the `listenerSummary` task, one call per ended "listen" call, strict JSON (`convex/ai/listenerMemory.ts`). The Listener persona is deliberately non-directive: reflect, open the door wider, never lecture or fix; the summary pass is deliberately non-addressing (it writes ABOUT the conversation, for the Listener to read later, never TO the person). The summary prompt also excludes **meta/product talk** — if the person talks about LifeGuide itself (a bug, a feature request, feedback on the app) rather than about their own life, that's left out of `summary`/`topics`/`open_threads` entirely, so a spoken feature request never comes back next call disguised as a personal open thread.

## 8. Data touched

**Owns:** none. **Writes:** `interviewSessions` (`experienceId: "listen"`, transcript, status, `summary` — ARI-23) and `experienceEvents`; triggers `coreFiles` writes via the Center. **Draws:** the realtime token (server-minted); the last call's `interviewSessions.summary` (via `latestListenSummaryInternal`, ARI-23) when minting a new call. Shapes in [`../../architecture/data-model.md`](../../architecture/data-model.md); the memory-backbone decision record is [ADR 0023](../../decisions/0023-listener-memory-backbone.md).
...
- **Memory depth beyond the last call (ARI-23, [ADR 0023](../../decisions/0023-listener-memory-backbone.md) §4).** v1 grounds the opening in exactly one prior summary. Last-N, or a rolling synthesized "where we are" memo that updates each call, is a real future direction — deliberately deferred as a bigger feature than a first cut needs.
- **A toss affordance.** The backend fully supports it (`interview.end` accepts `status: "tossed"`, and the memory backbone runs regardless — ADR 0023 §3), but no client UI calls it yet in this codebase.
