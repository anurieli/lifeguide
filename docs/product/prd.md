...
- **Daily Ritual: `built` (2026-07-12, v2 typed components same day).** Two ordered sequences of typed components (do / read / question / roadmap) hosted on Today: the morning is a primer sequence (Blueprint read → roadmap → question), the evening builds tomorrow morning's roadmap, plain to-dos ride the day's rail, per-day check state with completion history. The conduct half of the two bookends; the Journal is the reflective half. → [`features/daily-ritual.md`](features/daily-ritual.md)
- **The Blueprint: `built` (2026-07-12).** The person's editable conduct doctrine as a knowledge-base document (seeded from the 8-pillar Blueprint for Living), surfaced as a Settings card; the morning read resolves from it live. Core is the person; the Blueprint is conduct. → [`features/the-blueprint.md`](features/the-blueprint.md)
