...
- **Runs alongside, not through, the Listener's memory backbone** (ARI-23, [ADR 0022](../../decisions/0022-listener-memory-backbone.md)): `SpeakSurface` client-invokes `center.synthesizeSession` (identity — *who this person is*) on call end, while `interview.end` server-schedules the independent `ai/listenerMemory.summarizeSession` (conversation memory — *what we talked about*). A future toss can withhold this Center pass without losing that memory — the two were deliberately kept apart so tossing the Core filing never means forgetting the call happened.
