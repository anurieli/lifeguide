# Sessions (the living entry) — surfaced as **Dumps**
...
**Starting one.** The ➕ main action creates a fresh session and lands inside its empty document. On the **phone** (bottom bar's big center ➕) it is already recording (no separate recording screen); on **desktop** (the ➕ in the rail, below the nav items) the document opens ready to type, with the mic one click away — auto-arming the mic on desktop would throw a permission prompt unprompted. Tapping ➕ always starts a NEW entry; appending to an old one goes through the list. If the mic is denied, a small note says "Mic unavailable. Tap the page and type." and the document stays fully usable. A too-short take (~1s) saves nothing.
...
**The list.** The **Dumps** tab is the archive: pinned entries first, then newest by activity; relative time, a pin glyph, voice/photo count glyphs, the name (person-entered or AI), and the one-sentence living description (until the digest runs, the entry's first words). Tap to open. Swipe left to pin/unpin. Swipe right to delete: the row parks open with a Delete button, a second tap confirms. "Select" enters multi-select; picking 2+ entries shows "Merge N into one".
...
**Desktop.** Dumps is a rail tab (brain icon) and the rail carries the same ➕ main action; the same document behavior applies, and pin/delete appear as hover actions on list rows (no swipe). Desktop remains the command center. The phone bar is five even slots, Today · Board · ➕ · Dumps · account, with the ➕ the only dark element (active tabs get a light tint, never a solid block); Core, the Talk/Listener tab, and the Atmosphere music player (the audio engine itself, not just the orb) are desktop-only; the phone is capture-first. The separate **Thoughts** tab is gone on every device (ADR 0010): the flat stream surface merged into Dumps, and its component (`components/thoughts/ThoughtStream.tsx`) stays in the codebase unrouted, like the Brain Dump Lab before it.

## 3. Functions / actions

| Action | Trigger | What it does | Data touched |
|---|---|---|---|
| Start an entry | ➕ (phone bar / desktop rail) | `sessions.create`, opens the empty document; on the phone it auto-starts a take owned by `RecordingProvider` (desktop opens ready to type, mic one click away); save uploads the whole file as the first capture (`sourceMeta` carries device, durationMs, recordingStartedAt) | `sessions` (create), `captures` (create), `_storage` |
...
- ~~Whether the flat Thought Stream stays a top-level view once sessions dominate, or demotes to a filter.~~ **Resolved 2026-07-12 (ADR 0010): merged into Dumps; the tab is gone.** Whether a per-capture receipts/inspector *filter* view returns later is open.
- Desktop loose capture: with the stream composer unrouted, the board is the only desktop home for loose (non-session) captures; whether the Dumps list needs a quick loose-capture affordance is open.
