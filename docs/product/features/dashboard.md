...
This surface is the current "Today" screen ([`../../../components/today/Today.tsx`](../../../components/today/Today.tsx)), restructured 2026-07-12 around the ordered ritual: **a main column and a right-hand rail**. The main column: a time-aware greeting, the **Core state marker**, the **north star compass** (editable inline — the one write this surface issues), a **clock-locked** morning/evening toggle (only the beat `activeRitual` names is reachable — the other is a padlock with an "opens at 5:00 PM / 4:00 AM" hint, and its card is not rendered), the hosted **[Daily Ritual](daily-ritual.md) sequence card** (the ordered primer spine: read → roadmap → question, with its seal moment; the old inline one-move/tonight prompt cards are gone — they became question components inside the ritual), a Coach line, and the folded-in [Guide](guide.md) content (the Mirror + the pillars) under "Who you're becoming". The right rail (desktop ≥lg, sticky) holds **the Rituals rail** — the person's permanent `do` practices, out of the page's structure per Ariel's direction: one flat list (no morning/night sections), a sun/moon/sun-moon time marker per practice (morning / night / anytime), hover-✕ delete behind a warning, a single closable add at the bottom with the time-of-day picker, and the **keeping-up** last-7-days strip at its foot; on the phone the same panel renders as a card under the sequence. The separate **Today's log** card was removed 2026-07-13 (Ariel: "we don't need the second one") — a redundant second ritual list; only its keeping-up strip was kept, folded into the rail. The short-lived "Daily mantras" card was retired the same day it shipped: the readout now lives in the sequence's read step and its immersive reader.

**The Core state marker** adapts to whether the foundation is laid. While the Core is **incomplete** (`settings.blueprintStatus !== "complete"`) it is a **red nudge** at the top of the main column — "Finish the Core. Schedule a time — it takes about an hour." with an "It's the foundation everything else is built on. N/18 done." subline — the whole banner tapping through to the Core. The Core is the base of the whole system, so until it exists the surface says so plainly rather than burying it in a grey chip. Once the Core is **complete**, the nudge collapses to the quiet grey progress chip (`Core: 18/18 | Level N`) — the reminder is done, so it steps back.

**The keeping-up strip** (moved 2026-07-13 into the foot of the rituals rail; formerly the bottom third of a standalone "Today's log" card, `components/today/DayLog.tsx`, since removed) is a last-7-days strip answering "am I keeping up with the mornings and nights": one column per day, a gold dot when that morning was sealed and a dark dot when that night was closed, open rings otherwise. It shows the record, never a score or a streak count. The two parts of the old log that were dropped with it — "the parts of the day" (Morning ritual / Today's roadmap / Night ritual / Tomorrow's roadmap rows, made read-only the same day the beat locked to the clock, since there was nothing left to jump to) and "everything set down today" (the day's timestamped events) — were a redundant second reading of the ritual the sequence already walks; the surface keeps only the record strip. The proposed evolution makes the greeting identity-aware (greet by who you are, not just the clock) and makes the prompt-and-capture body proper Journal sessions rather than inline textareas that log directly.
...
| Host the rituals rail | Surface load | Renders the [Daily Ritual](daily-ritual.md)'s permanent practices on the right rail (desktop) or a card below (phone); checking, adding (morning/night/anytime), warned deletes, and the foot keeping-up strip are that element's own functions | Manual | that element's `ritualItems`, `ritualDays` (today + `rituals.history` for 7 days) |
...
- **The Dashboard no longer writes check-ins itself** (2026-07-12): the old inline one-move/tonight textareas became `question` components inside the [Daily Ritual](daily-ritual.md), whose answers publish `ritual_question` to `interactions` from that element. (The surface no longer renders those events back: the "Today's log" card that read `interactions` was removed 2026-07-13.)
...
- **Temptation to grow into a dashboard:** any pressure to add stats, streaks, or lists is resolved against the principle, depth is opened (by entering a session or surface), never dumped here. The keeping-up strip at the foot of the rituals rail walks this line deliberately (per Ariel): it is a **quiet record** — which mornings and nights got sealed this week — with no scores, no counters, no streak numbers. If a change would turn it into metrics, it loses. (The fuller "Today's log" card that also replayed the day's events was removed 2026-07-13 for being a redundant second ritual list; the record strip is what remains.)
...
- **Draws:** `mirror` (greeting, identity, Mirror card), `settings.northStar` and rhythm fields, `pillars` (the pillar blocks), `nodes` (per-pillar counts + Vision Board becoming-self), `sessions` (which beat / resume state), and — for the rituals rail's keeping-up strip — the Daily Ritual's `ritualItems` / `ritualDays` (today's check state and `rituals.history` for the 7-day strip). The hosted [Daily Ritual](daily-ritual.md) card reads and writes its own `ritualItems` / `ritualDays` through `convex/rituals.ts`; that data belongs to the Daily Ritual element, not to this surface. (The removed Today's log also read `interactions` via `interactions.forRange`; that draw is gone.)
