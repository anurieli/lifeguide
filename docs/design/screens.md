...
**On it, top to bottom:** a time-aware greeting; a Core progress chip (`x/18 · Level n`, with a "continue" link while incomplete); the **north star compass** (gold card with a compass mark, editable inline — tap "edit"/"write it", type, Save); a **Morning scroll / Night scroll** toggle (lands on the half of the day you're in; cutoffs in `lib/ritual.ts`) with, per beat: the **scroll card** (headed Morning scroll / Night scroll: the editable sequence with inline mantra readings, a muted `x/y` count, the gold all-done state with its single Seal-the-morning / Close-the-day confirm, and the sealed banner with the time; the morning scroll opens with the gold "✳ From last-night you" note card when one was left, and the night scroll's roadmap step carries the "✳ A note for morning-you" field; see [daily-ritual.md](../product/features/daily-ritual.md)) and the day's one prompt (**Morning:** "Today's one move" + Save; **Evening:** "Tonight" reflection + "Save & rest"; saves log `checkin_morning` / `checkin_evening` to `interactions`); a Coach whisper; then a "Who you're becoming" section — the **Mirror** (dark Coach card: value/theme tag chips + a noticed-summary) and each **pillar** with its tagged-thing count.
...
**Mobile (< `md`, 768px):** the rail folds down into a **fixed bottom tab bar** of five even slots: **Today · Board · ➕ · Thoughts · Goals**. The ➕ (dead center, raised above the bar) is the only dark element; active tabs get a light accent tint, never a solid block, so the bar always reads as one calm row. The ➕ starts a fresh entry already recording (see [sessions.md](../product/features/sessions.md)); Core is desktop-only, and the Talk/Listener tab and Atmosphere (the audio engine itself, not just the orb) are gone from the phone (2026-07-12): the phone is capture-first. The "L" wordmark is hidden, and the main surface gives up the bottom 64px so content is never trapped under the bar. **The account avatar (2026-07-13):** Goals took its bottom-bar slot, and rather than floating fixed in the top-right corner (where it collided with per-page header actions like the Thoughts "Select" button), the avatar is **baked into every page heading**: the shared `PageHeader` row ([`components/shell/PageHeader.tsx`](../../components/shell/PageHeader.tsx)) puts each surface's title block on the left, its own actions on the right, and — on mobile only — its own instance of the account menu (popup opens downward) at the row's far right, so every button has a fixed place and the avatar scrolls with the heading. Headings on Today, Core, Board (mobile list), Goals, Thoughts, and Settings all render through it; the open thought document stays pure capture (back-arrow header, no avatar). `PageHeader` reaches the shell's navigation through `ShellNavProvider` (React context from `AppShell`), so no surface threads `setView` props just for the menu; on desktop the row hides the avatar since the rail keeps its foot-of-rail instance. **Safe-area (2026-07-13):** installed as a PWA on a home-indicator phone, the bar reserves `env(safe-area-inset-bottom)` below its 64px row (`h-[calc(64px+env(safe-area-inset-bottom))]` + matching `pb-`, with `viewportFit: "cover"` already set) so the tabs sit above the home indicator instead of under it; the main surface subtracts that same inset from its height so nothing hides behind the taller bar. Desktop (≥ `md`): the vertical left rail — Today · Core · Board · **Thoughts** (booklet icon, `NotebookPen`) plus the **scribbler-pen main action** below the nav items (`PenLine`, hover "Think out loud"; 2026-07-12, ADR 0010 merged the flat stream into this surface — first labeled Dumps, renamed Thoughts the same day; on desktop the pen opens the fresh entry ready to type, mic one click away). The Feedback tab lives on both (2026-07-13): the desktop "Feedback?" tab shrinks on the phone to a minimalist icon-only nub on the right edge (draggable, bottom-anchored composer above the bar, photo paste/attach); it hides while the Coach sheet is open (see [feedback-widget.md](../product/features/feedback-widget.md)).
...
**One thing:** the conversation. It sits beside the work, never over it. **The one surface without it (2026-07-13):** the open thought document is pure capture — its own mic + photo pair rides the text caret there, and the whole Coach dock (buttons and window) steps aside until the person leaves the entry; every other surface keeps the corner cluster.
