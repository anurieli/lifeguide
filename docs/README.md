...
- [x] **Thought Stream (pipeline)** — the capture ingest pipeline (Whisper transcription, link fetch + extraction, image vision) with durable, re-analyzable raw storage. First slice of the MVP capture spine. The flat stream *surface* merged into Dumps on 2026-07-12 (ADR 0010).
- [x] **Sessions / Dumps** — the living entry and, since 2026-07-12 (ADR 0010), **the app's single capture surface**: the ➕ main action on the phone bar *and* the desktop rail opens a fresh dump (phone already recording; desktop ready to type); the entry is one continuous tap-and-type document (inline takes with pause/discard, photos, pasted links); a take keeps recording in the background across navigation; the person can name an entry (AI never overwrites a person's name) and the AI keeps a living description current (re-synthesized as content lands and on every open/leave — the line agents pull); the list has pin (swipe left), delete (swipe right), and multi-select merge; the phone bar is Today · Board · ➕ · Dumps (`sessions` + `captures.sessionId`, ADR 0008).
