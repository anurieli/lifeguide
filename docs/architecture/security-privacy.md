...
### Session persistence: a rolling 14-day window
The auth tokens live in cookies set by Convex Auth's Next.js middleware (`middleware.ts`). By default those are **session cookies** (no `maxAge`), which mobile browsers evict whenever the tab/process is killed — on the phone this forced a fresh Google sign-in on nearly every reload. `middleware.ts` therefore sets `cookieConfig.maxAge` to **14 days**; the middleware re-issues the cookies on each authenticated request, so the window is rolling — a person is only asked to sign in again after 14 days without opening the app. (Server-side session/refresh-token durations stay at the library's 30-day defaults, so the cookie is the binding limit.) This applies to Anonymous identities too, which makes them slightly more durable on mobile as well.
