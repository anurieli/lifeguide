...
**Only the beat you are in is reachable** (Ariel, 2026-07-12). The Morning/Evening control is not a free toggle — it is **locked to the clock**. From 4:00 to 16:59 the morning is live and the evening tab is locked (a padlock, muted, unclickable, with the hint "The night ritual opens at 5:00 PM"); from 17:00 through 3:59 it flips — the morning locks with "opens at 4:00 AM". At 17:00 you can no longer open the morning; at 4:00 you can no longer reopen last night. You only ever act on the ritual you are actually in. The locked ritual's contents are not rendered at all — there is nothing to peek at or edit out of its window. (The cutoffs are the same `NIGHT_START_HOUR` / `DAY_ROLLOVER_HOUR` as everything else in `lib/ritual.ts`; the lock is computed once per mount, so crossing a cutoff with the page open just needs a revisit.) Every submission inside a beat — each question answer, each roadmap walk, each seal — is still recorded to `interactions` and shown in the Today log, so the day's record is complete regardless of which beat is currently open.
