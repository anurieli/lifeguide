...
| Read completion history | Rituals rail renders | `rituals.history(sinceDay)` feeds the keeping-up strip and the gentle run at the foot of the rail | System | reads `ritualDays` |
| Show the gentle run | Rituals rail renders | `currentStreak(orderedDayKeys, keptDays)` (`lib/ritual.ts`) counts consecutive both-bookends-sealed days from the sealed-day history over `STREAK_WINDOW`; hidden at zero, penalty-free (ADR 0018) | System | reads `ritualDays` (via `history`) |
