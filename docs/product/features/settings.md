...
**Settings (any time).** A single calm surface (`components/settings/Settings.tsx`), titled "How I treat you," grouped into Daily rhythm, The Coach, Atmosphere, Your pillars, the AI hub ("AI — models, keys & activity"), Goals & Todoist, and Yours alone. The AI hub (ADR 0017) is the one place for everything model-related: every AI node with an inline model picker (the person's choice is a per-account override in `aiOverrides` that beats the `convex/ai/config.ts` default; "yours · reset" falls back), the per-provider keys, and the AI activity log — month-to-date calls/tokens/~cost and a per-call table (node, model, tokens in/out, cost, duration, ok/error) fed live from `aiLogs`. Every control writes immediately (no save button): toggles for morning/evening, segmented pickers for daily exercise, Coach tone, and reaching out; the [Atmosphere](atmosphere.md) controls (music on/off, autoplay, default mood); a pillar chip row with add (preset or custom); and sign-out. The north star is owned here but surfaced and edited inline on [Home (Today)](dashboard.md) (the compass card). Settings itself is reached from the **account menu at the bottom of the rail** (Settings / Account / Sign out), not a primary rail tab; the avatar no longer signs you out on click.
