...
**Navigation and overview (built).** As a board grows, the person is never lost in it. A **minimap** anchored to the bottom-right renders every node at scale with a live dashed indicator of the current viewport; clicking anywhere on it animates the viewport to that region. A **Gather** control (toolbar, layers icon) repacks every card into a compact, no-overlap grid — sorted by creation time, wrapping at roughly four standard cards wide, using the same `rectsOverlap` geometry as the server spiral placement — then animates to the new layout's center. A **Center the board** control (toolbar, crosshair icon) animates the viewport to the **whole-board centroid** — the average of every card's center (`boardCentroid` in `lib/geometry.ts`). The same landing happens on **every access of the board**: the surface stays mounted across nav, so when its tab flips active (or, on a restored board tab, when the node list first loads non-empty) the viewport re-centers on the centroid. Placing a capture from the Inbox also moves the camera: the card keeps its spiral-placed spot, and once the new node lands in the reactive list the viewport animates to it, so what was just placed is never off-screen. All viewport animation shares one `panTo` primitive (260ms cubic ease-out) in `useViewport`.
...
**Mobile (built).** The infinite pan/zoom canvas is a desktop surface — on a phone the gesture model fights the browser (pinch-zoom, scroll, and marquee all collide) and cards drift off-screen with no easy way back. So below the `md` breakpoint (≤767px, the same phone/desktop cutoff as `currentDevice`) the board renders as a plain, vertically stacked **list** of the same nodes instead of the board: no scrollable/zoomable canvas, no minimap, toolbar, or selection. Each card becomes a list row showing all of that item's data by type — image and generated-image cards show the picture with their caption/prompt and pillars (a generating card shows a spinner, a failed one its error note); text and quote cards show their text (quotes italic with the gold rule and attribution); link cards show the title, the destination as a tappable link, and the source; file cards show the filename, mime type, and a download link — each with its pillar chips and a dismiss (×) that soft-deletes the node. The **Inbox** sits at the top of the list as its own section: every undistilled/unplaced capture with its title/essence (or "distilling…") and **Place on board** / dismiss actions, so nothing is stranded off-canvas. Newest cards sort to the top; an empty board shows an invitation to add from a larger screen or record a brain dump. The switch is reactive (`useIsMobile`, an SSR-safe `matchMedia` hook) so rotating or resizing across the breakpoint swaps surfaces live. `MobileBoard` reuses the same `useNodes`/`useCaptures` data and the same place/dismiss mutations as the canvas, so the two surfaces are one board seen two ways.
...
| Center the board | Toolbar (crosshair); auto on board access | Animates viewport to the average of every card's center | Manual/auto | none (viewport only) |
| Center on placed capture | Placing from the Inbox | After `placeCapture`, the viewport animates to the newly placed node once it arrives in the node list | Auto | none (viewport only) |
...
- **Mobile list (built).** On a phone-width viewport the board is a read-and-manage list, not a canvas: every node as a stacked row (by type), the Inbox as a top section with place/dismiss, and per-row dismiss. No viewport, selection, edges, minimap, or toolbar. Reactive to the breakpoint (`useIsMobile`), so it swaps back to the canvas on resize/rotate past `md`.
...
- **Mobile board.** Below `md` (≤767px) the spatial canvas is replaced by a list surface (`MobileBoard`); the pan/zoom/selection/minimap chrome and its gesture collisions simply don't exist there. Edges and connection are not shown or editable on the phone list (the relationships still exist in the data and on desktop); the phone is for reading the board and placing/dismissing, not spatial authoring. An empty board shows an invitation rather than the canvas affordances.
- **Empty / single-node navigation.** Minimap is hidden on an empty board; Gather and Center-the-board are no-ops with zero nodes (an empty board has no centroid, so access-centering waits for the first node) and trivially correct with one.
