...
| Auto-forward to Linear | Runs automatically after every `submit` (system, no click) | If `FEEDBACK_AUTOFORWARD` is set and the row isn't already linked, creates an `agent:cody`-tagged Todo issue via `convex/linear.ts`, links the ticket (`linear{…}`), moves it to `pending`. No-ops (off by default) if the flag is unset | System | `feedback`, `_storage`, Linear |
...
**Piles + filters.** The panel splits tickets into three piles by status — **Needs you** (`open`), **In progress** (`pending`), **Dealt with** (`dealt_with`) — with live counts, and filters by type (All · Bugs · Tweaks · Features · Feedback).
...
**Auto-forward to Linear as `agent:cody`** (see [ADR 0031](../../decisions/0031-feedback-autoforward-to-linear.md)). Distinct from — and independent of — the manual Export to Linear button above: when the `FEEDBACK_AUTOFORWARD` env var is set on the Convex deployment, **every** feedback submission is forwarded to Linear automatically, immediately after `submit` inserts the row (`ctx.scheduler.runAfter(0, internal.linear.autoForwardFeedback, { feedbackId })` — scheduled, not awaited, so a slow or down Linear API never delays or blocks the user's submit). Where Export to Linear is a deliberate, human-triaged action, auto-forward is a **dumb, reliable pipe**: no filtering, no re-typing, no AI rewrite of the note — the created issue carries the type/view/submitter/verbatim text (plus any attached image) essentially as-is, tagged with the `agent:cody` label and set to **Todo**, so it flows straight into the Cody coding-agent pipeline (which does the interpreting) and surfaces in Slack. Off by default (unset `FEEDBACK_AUTOFORWARD` = no-op); flip it on with `npx convex env set FEEDBACK_AUTOFORWARD 1` on the deployment. Shares `LINEAR_API_KEY`/`LINEAR_TEAM_ID`/`LINEAR_PROJECT_ID` with the manual export path — both go through the same `createLinearIssue` GraphQL call in `convex/linear.ts`, factored out so the two paths don't duplicate the mutation. Idempotent on the same `linear` field the manual export uses: a row already linked (by either path) is skipped. Best-effort — a failed forward (missing key, Linear error) is logged and swallowed, never surfaced to the submitting user; the row's `linear` field stays unset so it remains visible to `adminList`/the `lifeguide-feedback` skill and can be exported manually or picked up by a later retry.
...
- **Linear labels/assignee at export** — v1 puts the type in the title/description and leaves labels/assignee/status to be set in Linear for the *manual* export path; the *auto-forward* path (above) does set `agent:cody` + Todo programmatically, so this is now partially resolved — only the manual button still leaves labels/assignee to be set by hand in Linear.
...
- **No retry loop yet** — a forward that fails (Linear down, bad key) stays unlinked until someone notices via the inbox/skill and exports it manually; an automatic retry (e.g. a periodic sweep of `open`/`pending` rows with no `linear` field) is a possible follow-up once auto-forward has run long enough to know how often it actually fails.
