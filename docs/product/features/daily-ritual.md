...
**Status:** built (v3, typed components — inline mantra + rotating morning journal) · **Element of:** the Sessions stream (the two calm bookends of the day) · **Owns:** `ritualItems`, `ritualDays`, `roadmapEntries`, `morningNotes`
...
**Morning.** Today lands on the Morning Scroll tab (cutoffs in `lib/ritual.ts`). If last-night you left a note, the scroll **opens with it**: a gold-washed "✳ From last-night you" card above the sequence, read-only. Below it, the sequence card walks its components in order — typically: **Read the Blueprint** (tap Read → the [immersive reader](#the-immersive-reader) fills the screen, reaching the end marks it read), **Read the mantra** (the day's mantra shown right there, inline — nothing to tap open; the circle is the acknowledgment), **Walk today's roadmap** (the list set last night, each entry tapped done as it happens; walking the last one checks the step), **The morning journal** (a reflection question drawn from the rotating morning bank, answered in place, typed or spoken — the daytime mirror of the night's Check out). The first unwalked step carries a faint gold wash — the sequence has a "you are here." Plain practices (drink the water) sit on the **rituals rail**: a right-hand panel on desktop, a card under the sequence on the phone. When every component AND every morning-assigned rail practice is done, the card turns gold and offers **Seal the morning**.

**The mantra step (Ariel, 2026-07-15).** A `mantra` component renders its line **inline**, in place — no Read button, because a mantra is short and read in a breath. Its words are the person's own fixed line, or (left empty) one drawn from the rotating **mantra pool** (`lib/mantras.ts`, seeded from the [Coach KB mantra pool](../coach-knowledge-base/mantras.md)); the pool rotates deterministically per ritual day, so the line **differs by the day** and is stable across devices until the 4am rollover. Reading it and tapping the circle checks the step. (The Read button stays exclusive to the long `read` — the Blueprint.)
...
**Defaults (new accounts):** morning — Read the Blueprint (from [the Blueprint document](the-blueprint.md)), Read the mantra (rotating mantra pool), Drink a glass of water (rail), Walk today's roadmap, The morning journal (rotating question). Night — Check out (rotating question), Set tomorrow's roadmap.

**Existing accounts** are offered the new components exactly once (`rituals.upgradeToSeedVersion`, marked by `settings.ritualsSeedVersion`, now at **v3** — the inline mantra + the rotating morning journal): appended after their existing items, only into rituals that still have items, deletable forever. Each addition is guarded by kind, so a v2 account (which already has a question and a roadmap) gains just the mantra, while an older account gains the mantra, the roadmap, and the rotating journal question together. A pre-existing fixed morning question is left as-is (rows are never rewritten); emptying its words in edit makes it rotate. The Blueprint read is adopted explicitly, per ritual — the **read from the Blueprint** button in edit mode (shown on morning and night alike, whichever one still lacks it), or the Settings card's "Read it each morning" for the morning specifically. `rituals.adoptBlueprintRead({ ritual })` is idempotent per ritual: morning and night each get their own blueprint-sourced read step, adopted and checked off independently, both resolving live from the same Blueprint document.
...
- **Roadmap, morning, nothing set:** "What does today start with? Set the first thing:" — the fast input is right there. (No mention of last night when nothing was left — an empty roadmap is irrelevant history, so the copy just points at today. Ariel, 2026-07-15.)
...
- **Legacy inline "read the mantra" items** (v1 `read` kind with inline `content`, behind the Read button) keep their words and their inline editing forever; nothing was migrated to the new `mantra` kind ([ADR 0011](../../decisions/0011-typed-ritual-components.md): additive only). A person who wants the inline, rotating behavior deletes the old read and adds a `mantra` (or takes the v3 upgrade's seeded one).

## 7. AI involvement

None at runtime. The question bank (`lib/questions.ts`) and the mantra pool (`lib/mantras.ts`) are both code-resident with deterministic per-ritual-day rotation (deliberately: no AI generation in this pass). The VoiceField's transcription/shaping applies to question answers as everywhere else. Parked: the Coach proposing components, drift signals off completion history, adaptive question/mantra selection, and the always-on ambient mantra surface (see the [Coach KB mantra pool](../coach-knowledge-base/mantras.md)).
...
**Owned:**
- `ritualItems`: `{ userId, ritual: morning|night|any, kind: do|read|mantra|question|roadmap, title, content?, source?, order, createdAt, updatedAt }` — the typed components (ADR 0011). `content` = a fixed mantra line / inline read words / a fixed question (empty mantras and questions rotate their pool/bank); `source` (reads) = `inline` | `blueprint`; `ritual: "any"` is a time-indifferent practice (kind `do` only, enforced server-side).
...
**Code:** `convex/rituals.ts`, `convex/roadmap.ts`, `convex/morningNote.ts`, `convex/blueprintDoc.ts`, `lib/ritual.ts` (cutoffs, day keys, spans, `nextRitualDayKey`, `msUntilRollover` + `formatCountdown` for the rail's reset timer, `currentStreak` + `keepingUpStatus` for the keeping-up calendar), `lib/questions.ts` (the question bank), `lib/mantras.ts` (the mantra pool), `components/today/RitualSequence.tsx` (the spine + edit + the inline mantra + the note's two faces), `components/today/RitualsRail.tsx` (the rituals rail + reset countdown), `components/today/ImmersiveReader.tsx` (the reader), wired in `components/today/Today.tsx`. Tests: `tests/ritual.test.ts`, `tests/questions.test.ts`, `tests/mantras.test.ts`, `tests/convex/rituals.test.ts`, `tests/convex/roadmap.test.ts`, `tests/convex/morning-note.test.ts`, `tests/convex/day-log.test.ts`.
