...
**Nodes are tiered by job, not run on one default model** (see [ADR 0014](../decisions/0014-per-node-model-tiering.md)). Synthesis and relationship nodes that build or steer the Core run on strong models — `curate` on `anthropic/claude-opus-4.8` (deepest, low-frequency), `coachReply` / `synthesis` / `journalPrompts` on `anthropic/claude-sonnet-5`; nodes that fan out or run often run mid-tier `anthropic/claude-haiku-4.5` (`center`, `brainDumpGraph`) and the realtime interview uses `gpt-realtime`; the bounded mechanical nodes (`distill`, `voiceShape`, `voicePrompts`, `extractImage`, `brainDumpSplit`, `sessionDigest`) stay on `openai/gpt-4o-mini`. All re-pointed nodes keep `provider: "openrouter"`, so key resolution and fallback are unchanged.
