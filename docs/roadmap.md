...
- **MVP — the capture spine.** Hosted + mobile-accessible. One pipeline, three front doors (brain dump, morning beat, night beat): `capture → decompose into atomic thoughts → sentiment + labels → propose+confirm routing → session stream OR a pillar-clean main thought`. Entities: **Session** (raw capture + metadata: time, day, duration, voice/text, and where known location, activity, source media), **Atomic thought** (text + sentiment + labels), **Main thought** (persistent, one pillar per thought). *In build (2026-07-12, ARI-24):* the capture valve shipped as [Thought Stream v1](product/features/thought-stream.md); the **Session** entity + the mobile front door (center ➕ opens a fresh entry already recording; phone bar is Today · Board · ➕ · Sessions) shipped as [Sessions](product/features/sessions.md) (ADR 0008; v2 2026-07-12 added the tap-and-type document, pin/delete swipes, and merge; v2.1 same day added pause/discard on a take and background recording that survives navigation; v3 same day merged the flat stream tab into the surface — the app's single capture surface, tab finally labeled **Thoughts** (briefly "Dumps"), with a desktop scribbler-pen main action ("Think out loud"), person-entered names the AI never overwrites, a living description refreshed on every open/leave, and seedable demo thoughts: ADR 0010). Decomposition, sentiment/labels, and propose+confirm routing remain. Two rules govern it: **clean storage, associative reading** (no forced cross-links written into storage; connections are found at read time) and the **transient ↔ durable membrane** (everything lands in the Session stream; only confirmed thoughts promote into the durable Brain via propose+confirm). Home screen is a plain, warm, identity-aware greeting. No read-back intelligence yet.
...
- **Product Tour (2026-07-18, ARI-19).** A guided, five-stop walkthrough of the app shell (Today, Core, Whiteboard, Coach, Settings) for an already-onboarded person — coachmarks anchored to real UI, skippable, resumable across reloads, restartable from Settings. Custom-built engine (ADR 0025), not a new schema table (rides three optional `settings` fields). See [`product/features/product-tour.md`](product/features/product-tour.md).
- **Today ritual.** The morning/evening beats wired to the interactions log.
- **Daily Ritual (2026-07-12, v2 same day).** From checklists to an **ordered primer sequence of typed components** (ADR 0011): a read resolved from the editable [Blueprint document](product/features/the-blueprint.md) with an immersive stop-scroll reader (ADR 0013), rotating-bank questions answered in place, and the **roadmap loop** — the evening builds tomorrow morning's roadmap, the morning walks it (ADR 0012). Plain to-dos moved to the day's rail (right panel on desktop). Per-day check state (4am rollover, ADR 0009); seals publish `ritual_completed`, answers publish `ritual_question`. See [`product/features/daily-ritual.md`](product/features/daily-ritual.md).
