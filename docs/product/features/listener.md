...
On `/speak`, a live two-color waveform shows who is speaking with the rolling transcript above it, and controls are **Pause** / **Mute** / **End**; **Close** leaves at any time. In the corner orb, the orb itself is the who-is-speaking signal (its glow color and swell). Above it floats a **minimalist translucent panel** (frosted, `bg-paper/75` + blur) that streams the Coach's current line as it speaks (with the last finished line held so it never blanks mid-thought) and shows your own words faintly while you talk — so you always know what it's doing without a full transcript view. Next to the status label, a **"what it knows"** toggle expands that panel to show the *exact* context the session was minted with (`mintRealtimeSession` now returns its `instructions` — the persona plus the person's own last-call memory or Blueprint status — so what the person sees is precisely what the model got, never a paraphrase). Under the orb sit three small icon buttons: **mute** (mic icon toggle), **toss** (✕ — discard the conversation: the session ends as `tossed`, the memory backbone still summarizes it per [ADR 0023](../../decisions/0023-listener-memory-backbone.md), but the Center never runs and no report shows, straight back to the pill), and **end-and-file** (✓). Ending runs the [Center](the-center.md) ("Filing what you shared…") and then shows the **filing report** — full-screen on `/speak`, a compact corner panel for the orb. The chat toggle sits right beside the idle pill in the corner row; it steps back while the orb owns the corner.
