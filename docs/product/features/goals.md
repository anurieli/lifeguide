...
The known-ids context is built by `ai/parse.ts`'s `buildGoalIntentMessages` as a single user-role turn, deliberately never a second `role: "system"` message. That variant shipped on landing day and made every single Coach turn 400 in production (OpenAI's `json_object` response mode requires the word "json" somewhere in the prompt; a second system message skipped the classifier's real configured system prompt, which is the only place that word appeared). Same-day hotfix; the classification call now receives one configured system prompt and one context-bearing user prompt, and degrades cleanly on any future provider failure. See `tests/goal-enrich-parse.test.ts` for the message-shape invariant and `tests/convex/goals.test.ts` for the API key stripping + description passthrough.

### Daily Ritual integration (ARI-144)
The Daily Ritual's roadmap loop is a **selection over these goals**, not a second to-do list. In the Night Scroll (setting tomorrow) and the Morning Scroll (adding to today), "Pull from your goals" picks an existing **open** `goalTasks` row (unchecked, not "waiting"); it never **creates** a task, so all task authorship stays here on the Goals board. Each pick is a pointer at the canonical task: while the link is live the roadmap shows the task's **current** content (a rename here shows through) and completion state; checking it in the Morning Scroll completes the task here and schedules the normal Todoist push. Removing it from the roadmap does **not** delete the task. Deleting the task **does** (`goals.deleteTask`): before the task disappears it **freezes** its latest content and latest canonical checked state into the linked roadmap rows, then severs the link so each row lives on as a plain frozen entry reading exactly as it did the instant before delete. The freeze only touches the deleting user's **own** linked rows (tenant-safe against corrupt cross-user links). Legacy roadmap rows stay readable as plain entries.
