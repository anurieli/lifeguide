...
- **Instructions:** [`persona.ts`](persona.ts) exports `LISTENER_INSTRUCTIONS`, the realtime system prompt. It is imported by `convex/ai/voice/index.ts` when minting a session whose `experienceId` is `"listen"`.
