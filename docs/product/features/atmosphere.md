...
A small orb sits in the side rail just above the account ("name") item by default. **It is draggable anywhere on screen** (the spot is remembered per-browser and re-clamped on resize so it can never strand off-screen). **Desktop-only since 2026-07-12:** below `md` the engine itself is silent, not just hidden — `attemptPlay` no-ops on a phone viewport (so autoplay never arms the first-tap gesture there) and crossing into the phone breakpoint mid-play pauses immediately (the mobile experience is capture-first; see [sessions.md](sessions.md)). At rest it shows a still dot on white; while music plays it fills with the current mood's color, runs a live soundwave, pulses a soft halo, and names the mood in small text beneath it, so the orb itself signals what is playing. On hover, a **play/pause affordance** fades in over the orb (a play glyph when paused, pause when playing).
...
It is mounted inside `AppShell` (wrapping the whole shell in `MusicProvider`), so it is present on every desktop surface (Today, Core, Board, Guide, Settings) and survives navigation between them; below the `md` breakpoint the player is hidden. It coexists with the **Coach dock** (right side on desktop) and the desktop rail. It is intentionally decoupled from the Context Bus so it can be enabled, disabled, or removed without touching the spine.
