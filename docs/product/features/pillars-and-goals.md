...
> **Superseded.** This doc pre-dates two things that actually shipped and diverged from what it describes: [ADR 0007](../../decisions/0007-file-system-on-the-human-and-the-center.md) turned pillars into folders (not the "cross-cutting tag" model described below — see [`pillars.md`](pillars.md) for the current pillar doc, extended again by ARI-11 / [ADR 0022](../../decisions/0022-identity-is-not-a-pillar.md)), and the Goals board shipped as **Orbit**, since replaced by the aspiration-to-goal model ([ADR 0029](../../decisions/0029-aspirations-goals-and-roadmap-steps.md): deadline-tiered goals linked to pillars via `pillarId`, AI-drafted `roadmapSteps`) — see [`goals.md`](goals.md) for the current canonical goals doc. Kept below for the original "goals across the Blueprint's horizons" idea, which was never built and isn't currently planned.
