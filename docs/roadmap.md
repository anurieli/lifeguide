...
- **Product Tour (2026-07-18, ARI-19).** A guided, five-stop walkthrough of the app shell (Today, Core, Whiteboard, Coach, Settings) for an already-onboarded person — coachmarks anchored to real UI, skippable, resumable across reloads, restartable from Settings. Custom-built engine (ADR 0022), not a new schema table (rides three optional `settings` fields). See [`product/features/product-tour.md`](product/features/product-tour.md).
