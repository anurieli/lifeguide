...
**Mobile (< `md`, 768px):** the rail folds down into a **fixed bottom tab bar** of five even slots: **Today · Board · ➕ · Dumps · account**. The ➕ (dead center, raised above the bar) is the only dark element; active tabs get a light accent tint, never a solid block, so the bar always reads as one calm row. The ➕ starts a fresh dump already recording (see [sessions.md](../product/features/sessions.md)); Core is desktop-only, and the Talk/Listener tab and Atmosphere (the audio engine itself, not just the orb) are gone from the phone (2026-07-12): the phone is capture-first. The "L" wordmark is hidden, the account popup opens upward, and the main surface gives up the bottom 64px so content is never trapped under the bar. Desktop (≥ `md`): the vertical left rail — Today · Core · Board · **Dumps** (brain icon) plus the same **➕ main action** below the nav items (2026-07-12, ADR 0010: the Thoughts tab merged into Dumps; on desktop the ➕ opens the fresh entry ready to type, mic one click away). The Feedback tab remains desktop-only.
