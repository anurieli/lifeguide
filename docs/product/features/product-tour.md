...
- **Anchors on, but does not modify the behavior of:** `data-tour="…"` attributes added to five existing elements — `Today`'s root, `Core`'s root, `Whiteboard`'s and `MobileBoard`'s roots (same attribute value, whichever is mounted), the "Talk to Coach" pill (`CoachOrb`, rendered by `CoachDock`), and the new "Restart tour" button in `Settings`. None of those elements' own logic changed; the attribute is purely a hook for `components/tour/useTourTarget.ts` to find and measure.
