...
**Mobile (< `md`, 768px):** the rail folds down into a **fixed bottom tab bar**, deliberately smaller than the desktop rail: **Today · ➕ · Sessions** plus the account avatar. The big center ➕ (screen-centered, raised above the bar) starts a fresh session already recording (see [sessions.md](../product/features/sessions.md)); Core, Board, and Thoughts are desktop-only, and the Talk/Listener tab and Atmosphere orb are gone from the phone (2026-07-12): the phone is capture-first. The "L" wordmark is hidden, the account popup opens upward, and the main surface gives up the bottom 64px so content is never trapped under the bar. Desktop (≥ `md`) is unchanged: the vertical left rail. The Feedback tab remains desktop-only.
