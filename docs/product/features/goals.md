...
- **Save validates immediately.** Hitting "Save" on the token runs `todoist.saveToken` (a Convex action), which makes a real, lightweight test call to the Todoist API (`GET /projects`) *before* persisting anything. A bad/expired token or an unreachable Todoist surfaces inline under the field (e.g. "Todoist rejected that token — double-check it's correct and has API access."); the token is only written to `apiKeys` once the test call succeeds. The raw token is never logged or echoed back to the client, on the success or error path.
- **Pull: "Sync Todoist"** button on the board header: fetches all projects and active tasks over the Todoist **unified v1 API** (`https://api.todoist.com/api/v1`) and reconciles. (The old REST v2 API — `…/rest/v2` — was sunset by Todoist and now returns HTTP 410 Gone, so all calls go through v1. v1 list endpoints return a paginated `{ results, next_cursor }` envelope capped at 200 items per page; `convex/todoist.ts`'s `todoistList()` helper unwraps the envelope and follows the cursor to the end.) Projects become goals (the Todoist Inbox maps to our Inbox, no card); sub-projects link `parentId`; tasks upsert by `todoistTaskId` (content/description/due/priority refresh; Todoist is authoritative for task content). Linked tasks missing from the active snapshot are marked done here. On failure the button shows the **actual** error thrown by `todoist.sync` (parsed via the shared `lib/convexError.ts` helper) rather than a hardcoded generic message, so a real cause (expired token, Todoist outage, a reconcile bug) is visible instead of always reading "check your token."
