...
### Dynamic sessions + thought maps — UX rework (ARI-18)
- [ ] **Toggle reads in plain words and goes gold:** open a Thoughts entry, tap the mode toggle (mic icon, "Brain dump") — it flips to a gold-filled "Conversation" pill with a speech-bubble icon; tap again and it flips back to "Brain dump". Tap target feels comfortable on the phone (not a tiny hit box).
- [ ] **The coach opens the conversation:** flip a fresh entry's toggle to "Conversation" — within a couple seconds, a short interviewer opener appears at the bottom of the thread (indented, thin gold left border, "Coach" tag) even though you haven't said anything yet; if the session already has prior material, the opener should reference it.
- [ ] **No double-greeting on toggle flapping:** flip the toggle off then back on quickly a few times — only one opener appears, not a stack of them.
- [ ] **Record button changes shape in Conversation mode:** with the toggle on Conversation, the mic control is now a bigger gold pill labeled "Speak" (idle) instead of the plain ambient mic; tap it — it becomes the discard/pause/stop pill, gold-accented, the stop button visibly pulsing while recording; after tapping stop, it briefly shows a spinner badge ("transcribing") before returning to "Speak". Flip back to Brain dump and confirm the mic returns to the old plain ambient icon-only button.
- [ ] **No dead air after finishing a take:** in Conversation mode, speak a real take and tap stop — a "listening…" line appears immediately at the bottom of the thread (in the coach's slot), then gets replaced by the real "thinking…" reply row once it lands, then by the actual reply. The placeholder never lingers after an error.
- [ ] **Voice replies feel fast, typed replies feel normal:** in Conversation mode, finish a voice take and note how long the reply takes to start (should feel close to ~2s after transcription lands, noticeably snappier than before); then type and commit a paragraph and confirm the reply still waits the longer ~8s debounce (so a burst of typing doesn't trigger a reply mid-thought).
- [ ] **Dynamic mode replies interleave after a voice take:** with the entry in Conversation, speak a real take and let it transcribe; an interviewer reply appears in the document, indented with a thin gold left border and a small "Coach" tag, positioned after the take in reading order.
...
- [ ] **Brain dump mode generates nothing new:** with the toggle left on "Brain dump", speak or type several takes — no new interviewer replies (and no opener, no "listening…") appear; existing replies from an earlier Conversation stretch, if any, still render.
- [ ] **The view switcher swaps the page, no popup:** in the header strip, tap "Thought Map" — the whole document body swaps to the map view in place (no overlay/backdrop, no close button); the composer and record controls are gone while the map view is showing. Tap "Thoughts" to swap back — your draft text (if any) and the feed are still there.
- [ ] **Auto-map with no tap required:** in a fresh session, speak/type a few real thoughts, wait roughly 30-40 seconds without touching the Thought Map tab, then open it — the map should already be there (or at worst show "mapping your thinking…" briefly), not the empty state, since it builds itself in the background.
- [ ] **Empty state before anything exists:** open the Thought Map tab on a genuinely empty/fresh session before any auto-map has run — it shows the calm empty state ("Your map builds itself shortly after you finish dumping.") with a "Map now" button, not an error.
- [ ] **Manual "Map now" and pending/error states:** tap "Map now" — it shows "mapping your thinking…", then renders as a collapsible **outline** on phone (no graph) with a bold root theme; a thought you retracted mid-ramble ("never mind, actually...") shows struck through and muted, not deleted.
- [ ] **Desktop shows the graph, fit to the screen:** open the same session's map on a wide window — the outline is replaced by an SVG graph that fits entirely within the panel with no scrolling needed, root node gold-bordered and bold, `leads_to` edges solid/gold/arrowed (heaviest), `part_of` edges light structural lines, `relates` edges thin and dashed, hovering a node shows its detail as a tooltip.
- [ ] **A big map still fits or scrolls honestly:** build (or fake, if feasible) a session with a large number of distinct thoughts — the graph should shrink to fit if it can stay readable, or clamp at a readable size and become pan/scrollable rather than rendering illegibly tiny text.
- [ ] **Remap after more talking updates the map:** add another take or two to the same session, open the Thought Map tab, tap "Remap" (small affordance near the map) — the map regenerates in place (same view, same session) and reflects the new content; it does not create a second map or duplicate the old one.
- [ ] **Old sessions unchanged:** open a Thoughts entry created before this shipped — it opens on "Brain dump" (no toggle flipped), shows no interviewer replies, and the Thought Map tab still works (auto-map or "Map now" as a fresh first-time generation).
...
- [ ] **Bar layout (v2.1):** at ~390px the bar is five even slots: Today · Board · ➕ (dead center, raised) · Sessions · avatar. Only the ➕ is dark; the active tab gets a light tint, never a big dark slab. No Talk tab. Core/Thoughts absent on the phone, present on desktop.
- [ ] **No music on the phone, at all (v2.1):** open the site fresh on the phone and tap around; no audio ever starts (autoplay used to arm on the first tap). Desktop autoplay/orb unchanged.
- [ ] **Pause/discard a take (v2.1):** while recording, the mic is a pill: ✕ · pause · save. Pause freezes the timer and the paused stretch is absent from the saved audio; ✕ asks "Sure?" then throws the take away (the entry stays; husk rules apply if it stays empty).
- [ ] **Background recording (v2.1):** start a take, switch to Today or the Board mid-take. A pill at the top shows the running timer; keep talking, tap the pill to land back in the entry, save, and confirm the transcript includes what you said while away.
