...
`goalTaskId?` makes the entry a **pointer at a canonical `goalTasks` row** instead of a free-form line (ARI-144): the roadmap is a *selection* over goals the person already tracks, not a second to-do list. `roadmap.addFromTask` sets it and writes `text` (and `note` from the task's description) as a **snapshot** captured at pick time. While the link is live, `roadmap.forDay` resolves the task's **current** `content` (a rename on the Goals board shows through) and its **current** `checked` state (a completion or reopen there is reflected, and a stale local `doneAt` never wins), and `roadmap.setDone` **mirrors a morning check back** to the canonical task (`checked` / `completedAt`) and reuses its Todoist push (`internal.todoist.pushChecked`). If the linked task is later **deleted** (or is not owned), the row falls back to the stored `text` snapshot and its own `doneAt`, rendering as a plain entry. **Legacy / free-form entries** (written before ARI-144, or via the retained `roadmap.add`) leave `goalTaskId` unset and behave exactly as before.
