...
- [x] **Daily Ritual** (2026-07-12, v2 same day): an ordered primer sequence of typed components (do / read / question / roadmap, ADR 0011) — the morning walks the Blueprint read (immersive reader, ADR 0013), the roadmap set last night, and the day's question; the evening builds tomorrow's roadmap (ADR 0012); plain to-dos on the day's rail; per-day check state with a 4am rollover (ADR 0009); seal-the-day completion history. The Blueprint itself is an editable knowledge-base document surfaced in Settings. See [`product/features/daily-ritual.md`](product/features/daily-ritual.md) + [`product/features/the-blueprint.md`](product/features/the-blueprint.md).
...
- [x] **Thought Stream (pipeline)** — the capture ingest pipeline (Whisper transcription, link fetch + extraction, image vision) with durable, re-analyzable raw storage. First slice of the MVP capture spine. The flat stream *surface* merged into the Thoughts tab on 2026-07-12 (ADR 0010).
- [x] **Sessions / Thoughts** — the living entry and, since 2026-07-12 (ADR 0010), **the app's single capture surface**, tab labeled **Thoughts** (booklet icon; briefly "Dumps" the same day): the main action — the phone bar's ➕ and the desktop rail's scribbler pen (hover "Think out loud"; also in the list header) — opens a fresh entry (phone already recording; desktop ready to type); the entry is one continuous tap-and-type document (inline takes with pause/discard, photos, pasted links); a take keeps recording in the background across navigation; the person can name an entry (AI never overwrites a person's name) and the AI keeps a living description current (re-synthesized as content lands and on every open/leave — the line agents pull); the list has pin (swipe left), delete (swipe right), and multi-select merge; seedable demo thoughts (two packed entries: voice + photos + text, client-painted media, no AI calls) show the surface lived-in; the phone bar is Today · Board · ➕ · Thoughts (`sessions` + `captures.sessionId`, ADR 0008).
