...
// One Big Thing card: title, context, an optional date. Click any of them to
// edit in place; the row also carries a "Promote to goal" and an archive
// affordance. Big Things carry NO roadmap and NO goal semantics until promoted.
function BigThingCard({
  item,
  onPromoted,
}: {
  item: Doc<"bigThings">;
  onPromoted: (goalId: Id<"goals">) => void;
}) {
  const update = useMutation(api.bigThings.update);
  const archive = useMutation(api.bigThings.archive);
  const promote = useMutation(api.bigThings.promote);
  ...
  const save = async () => {
    const t = title.trim();
    if (!t) {
      setTitle(item.title); // don't allow emptying the title
    } else if (t !== item.title || context !== (item.context ?? "") || date !== (item.date ?? "")) {
      await update({ id: item._id, title: t, context: context || null, date: date || null });
    }
    setEditing(false);
  };

  // Leave edit mode without persisting: restore every field from the current
  // stored item so an abandoned edit never lingers in local state or partially
  // persists. Used by both Escape and the Cancel button.
  const cancel = () => {
    setTitle(item.title);
    setContext(item.context ?? "");
    setDate(item.date ?? "");
  ...
  const doPromote = async () => {
    ...
    setPromoting(true);
    try {
      const goalId = await promote({ id: item._id });
      if (goalId) onPromoted(goalId);
    } finally {
      setPromoting(false);
    }
  };
  ...
  if (editing) {
    return (
      <div
        className="bg-card border border-line rounded-2xl p-4 flex flex-col gap-2"
        onKeyDown={(e) => {
      ...
      >
        <input
          ...
          placeholder="What's the big thing?"
          className="bg-paper border border-line rounded-lg px-3 py-2 text-[14px] font-semibold outline-none text-ink"
        />
        <textarea
          value={context}
          onChange={(e) => setContext(e.target.value)}
          placeholder="Context: what it is, where it stands (optional)"
          rows={2}
          className="bg-paper border border-line rounded-lg px-3 py-2 text-[13px] outline-none text-ink placeholder:text-ink-mute resize-none"
        />
        <label className="text-[12px] text-ink-mute flex items-center gap-2">
          Date
          <input
            type="date"
            value={date}
        ...
        <div className="flex gap-2">
          <button
            onClick={() => void save()}
            className="bg-ink text-white rounded-lg px-3.5 py-1.5 text-[13px] flex items-center gap-1.5"
          >
  ...
  return (
    <div className="group bg-card border border-line rounded-2xl p-4 flex flex-col gap-1.5 min-h-[104px]">
      <div className="flex items-start gap-2">
        <button
          onClick={startEditing}
          className="text-left font-semibold text-[14.5px] text-ink flex-1 leading-snug hover:text-accent transition"
        >
          {item.title}
        </button>
        {item.date && (
          <button
            onClick={startEditing}
            className="text-[10.5px] text-ink-mute border border-line rounded px-1.5 py-0.5 tabular-nums whitespace-nowrap hover:text-ink transition"
          >
            {item.date}
          </button>
        )}
      ...
      {item.context ? (
        <button
          onClick={startEditing}
          className="text-left text-[12.5px] text-ink-soft leading-snug line-clamp-3 hover:text-ink transition"
        >
          {item.context}
        </button>
      ) : (
        <button
          onClick={startEditing}
          className="text-left text-[12.5px] text-ink-mute italic hover:text-ink transition"
        >
          Add context…
        </button>
      )}
      <div className="flex items-center gap-3 mt-auto pt-1">
        <button
          ...
          disabled={promoting}
          className="text-[12px] text-ink-mute hover:text-accent transition flex items-center gap-1.5 disabled:opacity-50"
        >
          <ArrowUpCircle className="w-3.5 h-3.5" />
          {promoting ? "Promoting…" : "Promote to goal"}
        </button>
        <button
          onClick={() => void archive({ id: item._id })}
          aria-label="Archive this big thing"
          className="text-[12px] text-ink-mute hover:text-red-500 transition flex items-center gap-1.5 hover-reveal"
        >
...
// Big Things: the active commitments (pending meetings, ongoing projects,
// obligations in flight) that occupy time and mental space. Sits prominently
// above the Goals/Aspirations gallery. Quick-capture, edit, archive, and an
// explicit "Promote to goal"; no AI roadmap runs until a promotion happens.
function BigThingsSection({ onPromoted }: { onPromoted: (goalId: Id<"goals">) => void }) {
  const items = useQuery(api.bigThings.list, {});
  const create = useMutation(api.bigThings.create);
  ...
  const add = async () => {
    const t = title.trim();
    if (!t) return;
    await create({ title: t, context: context || undefined, date: date || undefined });
    setTitle("");
    setContext("");
    setDate("");
    setOpen(false);
  ...
  return (
    <div className="mb-8">
      <div className="flex items-baseline justify-between mb-2 gap-3 flex-wrap">
        <div>
          <div className="text-[12px] font-semibold text-ink">Big Things</div>
          <div className="text-[11.5px] text-ink-mute">
            Pending meetings, ongoing projects, commitments in flight.
          </div>
        </div>
        {!open && (
          <button
            onClick={() => setOpen(true)}
            className="text-[12.5px] text-ink-mute hover:text-ink transition flex items-center gap-1.5"
          >
            <Plus className="w-3.5 h-3.5" /> Add a big thing
          </button>
        )}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {open && (
          <div
            className="bg-card border border-dashed border-line rounded-2xl p-4 flex flex-col gap-2"
            onKeyDown={(e) => {
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              <button onClick={cancelCreate} className="text-[13px] text-ink-mute px-2">
                Cancel
              </button>
            </div>
          </div>
        ...
        {(items ?? []).map((item) => (
          <BigThingCard key={item._id} item={item} onPromoted={onPromoted} />
        ))}
        {items && items.length === 0 && !open && (
          <button
            onClick={() => setOpen(true)}
            className="border border-dashed border-line rounded-2xl p-4 min-h-[104px] text-ink-mute hover:text-ink hover:border-ink-mute transition flex items-center justify-center gap-2 text-[13px]"
          >
            <Plus className="w-4 h-4" /> Capture a meeting, project, or commitment
          </button>
        )}
      </div>
    </div>
      ...
      <div className="max-w-[1080px] mx-auto px-5 pt-8 pb-24 md:px-10 md:pt-11">
        ...
        <div className="flex flex-col lg:flex-row-reverse gap-5 items-start">
          ...
          <div className="flex-1 w-full min-w-0">
            <BigThingsSection onPromoted={(goalId) => setExpandedGoalId(goalId)} />
