...
export default defineSchema({
  ...
  ritualItems: defineTable({
    ...
    kind: v.union(
      ...
      v.literal("question"),
      v.literal("roadmap"),
      v.literal("tidbit"), // a daily inspirational quote, generated by the Haiku agent
    ),
    title: v.string(),
    content: v.optional(v.string()), // read/mantra/tidbit: inline text · question: fixed prompt
  ...
  // The Horizons ladder (docs/product/features/horizons.md): the object-oriented spine
  // of a person's plan — nested goals from the far 5-year vision down to today. One row
  // per rung per period. `scope` is the rung; `period` addresses which instance:
  //   standing rungs (five_year|one_year|one_month) → period "std" (one evolving line),
  //   weekly → the Monday-of-week key, daily → the ritual day key (lib/horizons.ts).
  // Standing rungs hold a single line (order 0); weekly/daily hold up to 3 ordered,
  // checkable goals ("the most important thing, plus two more"). North Star is NOT here —
  // it stays in settings.northStar and renders as the ladder's crown.
  horizons: defineTable({
    userId: v.id("users"),
    scope: v.union(
      v.literal("five_year"),
      v.literal("one_year"),
    ...
    period: v.string(), // "std" | week key | ritual day key
    text: v.string(),
    order: v.number(),
    doneAt: v.optional(v.number()), // weekly/daily goals can be checked off
    createdAt: v.number(),
    updatedAt: v.number(),
  }).index("by_user_scope_period", ["userId", "scope", "period", "order"]),

  // A daily generated "tidbit" for the morning scroll — currently an inspirational
  // QUOTE chosen for the person from their Core by the cheap Haiku `dailyQuote` agent
  // (convex/ai/dailyQuote.ts, registered in the AI hub). One row per (user, day, kind),
  // cached so the agent runs at most once per day; `status` tracks the async generation.
  // `kind` is a union so future tidbit kinds (an image, a fact) slot in without a rewrite.
  // See docs/product/features/daily-tidbit.md.
  dailyTidbits: defineTable({
    userId: v.id("users"),
    day: v.string(), // ritual day key (lib/ritual.ts)
    kind: v.union(v.literal("quote")),
    status: v.union(v.literal("pending"), v.literal("done"), v.literal("error")),
    text: v.optional(v.string()), // the quote body
    attribution: v.optional(v.string()), // who said it
    model: v.optional(v.string()), // which model produced it (provenance)
    error: v.optional(v.string()),
  ...
  // is a light, optional pointer at a Horizons standing rung (docs/decisions/0029) —
  ...
  interviewSessions: defineTable({
    ...
    // tossed alike — see ADR 0023), distinct from the Center's identity filing
  ...
  // docs/product/features/whats-new.md, ADR 0026). `view` is a Rail `View` key
