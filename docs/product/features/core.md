...
### Modes of the Core
The Core can be filled three ways today — with a fourth, the **journal**, accepted as the main long-run path but not yet built ([ADR 0030](../../decisions/0030-the-journal-is-the-cores-incremental-intake.md): one unanswered slot a day, answered in the daily ritual). All of them write the **same** answers (`coreResponses`, keyed by the 18 blueprint keys) so progress carries across modes. The three built ones are *sit-down* modes — the person must choose to come to the Core and work on it; the journal is the one that fills it without him ever making that trip. The mode machine is a small pure reducer, `lib/core/mode.ts` (`coreModeReducer`, `grid | zen | conversational`), driven by `components/core/Core.tsx` via `useReducer`; every mode can reach every other mode directly — there is no gating and no "must exit through grid" rule (`tests/core-mode.test.ts`). See [ADR 0024](../../decisions/0024-core-conversational-mode-engine.md).
...
- **Conversational** (`components/core/ConversationalCore.tsx`) — building the Core by talking. **Built (ARI-2).** A real WebRTC voice call, built on the same spine as onboarding's `VoiceInterview` and the Listener's `SpeakSurface` (`useRealtimeVoice` over an `interviewSessions` row, `experienceId: "core"`, minted by `ai/voice.mintRealtimeSession`). The conversation is free-flowing — turns are never tagged to a specific question key in real time. When the call ends, `ai/synthesizeInterview.synthesizeInterview` (the same action onboarding's voice interview already uses) reads the whole transcript and maps it onto the 18 blueprint keys in one pass, writing only the keys still empty; an authored answer is never silently overwritten, and a spoken answer that conflicts with one on file is surfaced in the post-call summary, not applied. `mintRealtimeSession` personalizes the persona for `"core"` sessions (`buildCoreInstructions`, `convex/ai/voice/index.ts`) from the person's current answers, so it never re-asks a settled question and, once all 18 are filled, shifts to a "reflect or revise" framing instead of interrogating a checklist. Honors the concept's "manual AND Coach are both first-class" principle.

**Switching:** the grid offers two pills side by side — **Talk it through** (→ Conversational) and **Zen** (ARI-106: the grid entry into Conversational, so the spoken mode is reachable in one click instead of only via Zen's hover rail); inside Zen, the rail header offers **Talk** (→ Conversational) and **Exit Zen** (→ grid); Conversational's header offers **Zen** and **Grid** at all times, including mid-call — switching away ends the call first (so it's synthesized, not silently dropped) and completes the mode switch immediately without waiting on that to finish; since synthesis writes through `core.save` and both the grid and Zen subscribe live, whatever it fills in appears reactively once it lands. No data is lost switching, since all three bind the same `coreResponses`.
...
- **Incremental, through the journal — the main path** ([ADR 0030](../../decisions/0030-the-journal-is-the-cores-incremental-intake.md)). Every day the journal asks **one** thing the Core still doesn't know, in the ritual he already walks, and the answer writes to these same `coreResponses` keys. He is journaling, not filling a form — but `bpCount/18` moves because of it. **The Core is built by living, one entry at a time**; the hour-long sit-down is optional, not the price of entry. Binding rules for anyone implementing this (one a day, never re-ask a settled question, one store, his words win, always skippable) are in the ADR. Tracked by [ARI-120](https://linear.app/cuttheedge/issue/ARI-120).
- **Ambient.** The rest accrues without him answering anything at all. A capture placed on the Vision Board, a Coach exchange, a morning or night session, an image added to the Future Self: each emits distilled text, and the Coach curates that signal into the Core. Over weeks the backbone fills itself.
