...
| Host the rituals rail | Surface load | Renders the [Daily Ritual](daily-ritual.md)'s permanent practices on the right rail (desktop) or a card below (phone); checking, adding (morning/night/anytime), and warned deletes are that element's own functions | Manual | that element's `ritualItems`, `ritualDays` |
