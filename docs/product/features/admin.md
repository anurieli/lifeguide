...
- **Feedback / Escalations** inbox (`components/feedback/FeedbackInbox.tsx` — a self-contained, embeddable panel): in-app feedback, live (Convex-reactive). For the **owner** this is **every user's** feedback (the support inbox); for anyone else it is only their own. Each row shows the type, route/view, time, the **submitter** (name + email, or "anonymous"), the note, an error count + expandable error log, and the page snapshot + attached-photo thumbnails.
  - **Piles + filters.** A segmented control splits tickets into three piles — **Needs you** (`open`), **In progress** (`pending`), **Dealt with** (`dealt_with`) — each with a live count; a second row filters by type (**All · Bugs · Tweaks · Features · Feedback**). The header keeps a red open-count badge.
  - **Reply.** When the submitter has an email, **Reply** opens a `mailto:` in your own mail client, pre-addressed and quoting their note — replies go from your real email, no provider needed. Clicking it moves the ticket to **In progress** (`markPending`). (Inline sending via Resend is a deferred follow-up.)
  - **Export to Linear.** **Export to Linear** opens a small inline form (issue name, prefilled from the note; urgency → Linear priority), then creates a real Linear issue in the configured project via `convex/linear.ts` — carrying the note, the page context, and the photo/screenshot uploaded as a real Linear asset. On success the ticket links out to the card (its `ARI-…` identifier becomes a chip) and moves to **In progress**. Requires `LINEAR_API_KEY` in the Convex deployment; the destination team/project default to LifeGuide's and are env-overridable.
  - **Close.** **Dealt with** moves a row to the closed pile; **Reopen** brings it back to **Needs you**.
  - Backed by `convex/feedback.ts` (`listAll`/`markPending`/`resolve`/`reopen`/`markExported`, owner-aware) + `convex/linear.ts` (`exportFeedback` action). Submissions come from the [Feedback Widget](feedback-widget.md); the lifecycle + Linear rationale live in [ADR 0019](../../decisions/0019-feedback-to-linear.md).
- **Back to app** link.
